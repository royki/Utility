...
sys.path.append(os.path.dirname(CURRENT_DIR)+'/regression/qa_config')
...
def convertDuration(file):
	...
	minutes = seconds // 60
	seconds %= 60
	text = text.replace(duration+"s", str(hours).zfill(2)+"h:"+str(minutes).zfill(2)+"m:"+str(seconds).zfill(2)+"s")
	with open(file, 'w') as f:
		f.write(text)
...
#Call to methods
setCssStyle(reportPath)
cutClassName(reportPath, prefix)
convertDuration(reportPath)